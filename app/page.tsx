...
export default function FuturisticRadhika() {
  ...
  const MarkdownComponents: Components = useMemo(
    () => ({
      h1: ({ children }) => (
        <h1
          className={`text-base lg:text-lg mb-2 lg:mb-3 text-gray-900 dark:text-gray-100 ${
            uiStyle === "pixel"
              ? "text-xs lg:text-sm font-bold pixel-font"
              : "font-semibold"
          }`}
        >
          {children}
      ...
      h2: ({ children }) => (
        <h2
          className={`text-sm lg:text-base mb-1.5 lg:mb-2 text-gray-900 dark:text-gray-100 ${
            uiStyle === "pixel"
              ? "text-xs lg:text-xs font-bold pixel-font"
              : "font-semibold"
          }`}
        >
          {children}
      ...
      p: ({ children }) => (
        <p
          className={`mb-2 lg:mb-3 last:mb-0 text-sm lg:text-base text-gray-700 dark:text-gray-300 leading-relaxed ${
            uiStyle === "pixel" ? "text-xs lg:text-xs pixel-font" : ""
          }`}
        >
          {children}
        </p>
      ...
      ul: ({ children }) => (
        <ul
          className={`mb-2 lg:mb-3 space-y-1 text-sm lg:text-base text-gray-700 dark:text-gray-300 ${
            uiStyle === "pixel"
              ? "list-none text-xs lg:text-xs pixel-font"
      ...
      li: ({ children }) => (
        <li
          className={`text-sm lg:text-base text-gray-700 dark:text-gray-300 ${
            uiStyle === "pixel"
              ? 'text-xs lg:text-xs pixel-font before:content-["▶_"] before:text-cyan-600 dark:before:text-cyan-400'
              : ""
          }`}
        >
          {children}
      ...
      strong: ({ children }) => (
        <strong
          className={`text-gray-900 dark:text-gray-100 ${
            uiStyle === "pixel" ? "font-bold pixel-font" : "font-semibold"
          }`}
        >
          {children}
        </strong>
      ...
      pre: ({ children }) => (
        <pre
          className={`p-2 lg:p-3 mb-2 lg:mb-3 overflow-x-auto ${
            uiStyle === "pixel"
      ...
      blockquote: ({ children, ...props }) => (
        <blockquote
          ...
          className={`border-l-4 border-cyan-500 pl-3 lg:pl-4 italic mb-2 lg:mb-3 text-sm lg:text-base text-gray-600 dark:text-gray-400 ${
            uiStyle === "pixel"
              ? "pixel-font border-cyan-600 dark:border-cyan-400"
              : ""
          }`}
        >
          {children}
    ...
    () => ({
      ...
      onError: (error: Error) => {
        ...
        if (
          ...
          setError(
            `${errorMessage} Please set up your ${PROVIDERS[provider].name} API key.`
          );
        }
      },
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const savedApiKeys = localStorage.getItem("Intello AI-api-keys");
      if (savedApiKeys) {
        try {
  ...
  const handleSaveApiKey = useCallback(() => {
    ...
    localStorage.setItem("Intello AI-api-keys", JSON.stringify(updatedApiKeys));
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        isProviderMenuOpen &&
        !target.closest('[data-provider-menu="true"]')
      ) {
        setIsProviderMenuOpen(false);
      }
    };
...
`}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2 sm:space-x-3 flex-shrink-0">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className={`text-gray-800 dark:text-gray-300 transition-colors p-2 ${
                      uiStyle === "pixel"
                  ...
                  >
                    <a
                      href="https://github.com/Nyayabrata01/Intello-AI-"
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    <div className="flex-1">
                      ...
                      >
                        <div className="flex items-center space-x-2">
                          ...
                          >
                            {uiStyle === "pixel"
                              ? "Intello AI  IS PROCESSING..."
                              : "Intello AI  is thinking..."}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <div className="relative" data-provider-menu="true">
                        <button
                          ...
                          className={`h-6 px-2 text-xs flex items-center gap-1 font-medium transition-colors ${
                            uiStyle === "pixel"
                        ...
                        {isProviderMenuOpen && (
                          <div
                            className={`absolute right-0 bottom-full mb-1 w-40 py-1 z-50 ${
                              uiStyle === "pixel"
                            ...
                            {Object.entries(PROVIDERS).map(
                              ([key, providerData]) => (
                                <button
                                  key={key}
                                  onClick={() =>
                                    handleProviderChange(key as Provider)
                                  }
          ...
          <div
            className={`hidden xl:flex w-80 flex-col ${
              uiStyle === "pixel"
        ...
        <Dialog open={isApiKeyDialogOpen} onOpenChange={setIsApiKeyDialogOpen}>
          ...
          >
            <DialogHeader>
              ...
              >
                To use {PROVIDERS[selectedProvider]?.name}, please enter your
                API key. It will be stored securely in your browser.
              </DialogDescription>
            </DialogHeader>
